...
def graficar_tiempos(lista_metodos_ord, k=1001, l=20):
    volumen = [n for n in range(1, k, l)]
    plt.figure(figsize=(10, 6))

    for metodo_ord in lista_metodos_ord:
        
        tiempos = medir_tiempos(metodo_ord, volumen)
        ...
        plt.plot(volumen, tiempos, marker='o', label=metodo_ord.__name__)
...
if __name__ == '__main__':
   ...
   graficar_tiempos([Burbuji])
   graficar_tiempos([ordenamientoRapido])
   graficar_tiempos([radix_sort])
   graficar_tiempos([Burbuji, ordenamientoRapido, radix_sort])
